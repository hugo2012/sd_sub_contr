sap.ui.define([
        ...
        */
        _loadVariants: async function () {
            ...
            // 1. Ensure Standard/Default variant exists
                var bHasStandard = oVariantData.some(function (v) { return v.key === "Default"; });
                if (!bHasStandard) {
                    oVariantData.unshift({ key: "Default", text: "Standard", author: "SAP" });
                }

                // 2. Determine default variant key
        
                var oDefaultVariant = oVariantData.find(function (v) { return v.isDefault === true; });
                var sDefaultKey = oDefaultVariant ? oDefaultVariant.key : "Default";

                if(!oDefaultVariant){
                   oDefaultVariant =   oVariantData.find(item => item.key === sDefaultKey);
                }
               
                // 3. Update view model state
                oModel.setProperty("/variants", oVariantData);
                oModel.setProperty("/selectedKey", sDefaultKey);
                this._oVM.setDefaultKey(sDefaultKey);
                // 4. Apply selected variant state to table
                this._applyVariant(oDefaultVariant);
   ...
   /*          if (oVariantData.length) {
        ...
        fnGetFilterVaules: function () {
            ...
            oFilterCondData.prefillDelQty = Formatter._fnConvertTruefale(this.byId("idcb_pre_fill_delQuantity").getSelected());
            oFilterCondData.componentstock = Formatter._fnConvertTruefale(this.byId("idcb_componentwithStock").getSelected());
        ...
        onValueHelpRequestAuto: function (oEvent) {
            ...
            switch (oControlname) {
                ...
                case "idip_shipping_point":
                    ...
                    data.Field2 = this.getResourceBundle().getText("lbl_description")
                    data.Field3 = this.getResourceBundle().getText("lbl_plant")
                    data.Field4 = this.getResourceBundle().getText("lbl_loadingGroup")
                    arrayFieldsLabel.push(data);

                    data = new Array();
                    data.Field1 = "ShippingPoint"
                    data.Field2 = "ShippingPointName"
                    data.Field3 = "Plant"
                    data.Field4 = "LoadingGroup"
                    arrayColumns.push(data);

                    var _inlineCount = 0;
                    var arrFieldsSet = [];
                    var data = new Array();
                    if (this.byId("idip_plant").getValue()) {
                        aFilter.push(new sap.ui.model.Filter(arrayColumns[0]["Field3"],
                            sap.ui.model.FilterOperator.Contains,
                            this.byId("idip_plant").getValue()));
                    }
                    this.getService().getShippingPointSet(aFilter).then(function (oData) {
                        ...
                        const aPreFilters = [];
                        const sPlantVal = this.byId("idip_plant").getValue();
                        if (sPlantVal && sPlantVal.length > 0) {
                            aPreFilters.push({ path: arrayColumns[0]["Field3"], operator: sap.ui.model.FilterOperator.Contains, values: [sPlantVal] });
                        }
                        this.onValueHelpInputPopup(arrayFieldsLabel, arrayColumns, arrFieldsSet, this.oControl, this.getResourceBundle().getText("lbl_shipping_point"), _entityName, _inlineCount, aPreFilters);
                    }.bind(this), function (oError) {
                        this.fnSetBusyIndicatorOnDetailControls(this.getView().byId("idip_shipping_point"), false);
                    }.bind(this));
                ...
                case "idip_component":
                    ...
                    data.Field3 = this.getResourceBundle().getText("lbl_description");
                    data.Field4 = this.getResourceBundle().getText("lbl_loadingGroup");
                    data.Field5 = this.getResourceBundle().getText("lbl_shipping_point");
                    data.Field6 = this.getResourceBundle().getText("lbl_plant");
                    arrayFieldsLabel.push(data);
                    ...
                    this.getService().getCompoSet(aFilter).then(function (oData) {
                        ...
                        var _arrPrefliter = {};
                        var _tblPrefliter = [];
                        if (this.getView().byId("idip_shipping_point").getValue()) {
                            let _f4Value = this.getView().byId("idip_shipping_point").getValue();
                            if (_f4Value.length > 0) {
                                _arrPrefliter = {
                                    path: arrayColumns[0]["Field5"],
                                    operator: sap.ui.model.FilterOperator.Contains,
                                    values: [_f4Value]
                                };
                                _tblPrefliter.push(_arrPrefliter);
                        ...
                        if (this.getView().byId("idip_plant").getValue()) {
                            // let _f4Value = this.getView().byId("idip_warehouse_number").getValue();
                            var _f4Value = this.getView().byId("idip_plant").getValue();
                            if (_f4Value.length > 0) {
                               _arrPrefliter = {
                                        path: arrayColumns[0]["Field6"],
                                        operator: sap.ui.model.FilterOperator.Contains,
                                        values: [_f4Value]
                                    };
                                    _tblPrefliter.push(_arrPrefliter);
        ...
        onValueHelpInputPopup: async function (_oLabels, _oColumns, _oData1, oControl, _title, _entityName, _inlineCount, _otblPrefliter) { // debugger;
        ...
        fnApplyFilerValue: function (oData) {
            ...
            this.fnSetDefaultChkValue(oData.prefillDelQty, this.byId("idcb_pre_fill_delQuantity"));
            this.fnSetDefaultChkValue(oData.componentstock, this.byId("idcb_componentwithStock"));
        }
        ,
        fnSetDefaultChkValue: function (a, o) {
            if (a) {
                let bSelected = (a === "X") ? true : false;
                if (a.length > 0) {
                    o.setSelected(bSelected);
                }
                else {
                    o.setSelected(bSelected);
                }
        ...
        onInputChange: function (oEvent) {
            ...
            if (oEvent.oSource.getId().includes("idip_plant") 
                || oEvent.oSource.getId().includes("idip_shipping_point"))
            {
                if (oEvent.oSource.getValue().length < 1) {
                    oEvent.oSource.setValueState(sap.ui.core.ValueState.None);
                    oEvent.oSource.setValueStateText("");
                }
                 if (oEvent.oSource.getValue().length >= 4) {
                   this.onSubmitInput(oEvent);
                }
            }
        },
       ...
       onSubmitInput: function (oEvent) {
            ...
            // Fix: Retrieve binding context safely first, then get the path
            var oContext = oSource.getBindingContext(); 
            var sPath = oContext ? oContext.getPath() : "";
           // var idx = sPath ? sPath.split("/")[2] : "";
            var sSelectedKey = oSource.getValue();
            var sControlName = sId.split("--")[2] || sId;
            var oControl = this.byId(sControlName) || oSource;
            var aFilters = [];
            this._oVM.setModified(true);
            // --- 1. PLANT CHECK ---
            if (sId.includes("idip_plant")) {
                aFilters.push(new sap.ui.model.Filter({
                    path: "Plant", 
                    operator: sap.ui.model.FilterOperator.EQ, 
                    value1: sSelectedKey
                }));
                if (sSelectedKey.length >= 4) {
                    this.fnSetBusyIndicatorOnDetailControls(oControl, true);
                    this.getService().getPlantSet(aFilters).then(function (oData) {
                        this.fnSetBusyIndicatorOnDetailControls(oControl, false); // Turn off busy indicator
                        
                        if (oData && oData.oData && oData.oData.length > 0) {
                            oControl.setValueState(sap.ui.core.ValueState.None);
                            oControl.setValueStateText("");
                        } else {
                            oControl.setValueState(sap.ui.core.ValueState.Error);
                            oControl.setValueStateText(this.getResourceBundle().getText("err_invalid_plant"));
                        }
                    }.bind(this)); // Bound 'this' to retain controller context
                }
            } 
            // --- 2. SHIPPING POINT CHECK ---
            else if (sId.includes("idip_shipping_point")) {
                aFilters.push(new sap.ui.model.Filter({
                    path: "ShippingPoint", 
                    operator: sap.ui.model.FilterOperator.EQ, 
                    value1: sSelectedKey
                ...
                if (sSelectedKey.length >= 4) {
                    this.fnSetBusyIndicatorOnDetailControls(oControl, true);
                    this.getService().getShippingPointSet(aFilters).then(function (oData) {
                        this.fnSetBusyIndicatorOnDetailControls(oControl, false); // Turn off busy indicator
                        if (oData && oData.oData && oData.oData.length > 0) {
                            oControl.setValueState(sap.ui.core.ValueState.None);
                            oControl.setValueStateText("");
                            ...
                            oControl.setValueState(sap.ui.core.ValueState.Error);
                            oControl.setValueStateText(this.getResourceBundle().getText("err_invalid_shipping_point"));
                        }
                    }.bind(this)); // Bound 'this' to retain controller context
                }
            }
